...
programmer, and co-founder of [Websthan](https://websthan.online), a web design
and digital marketing agency in Dhaka, Bangladesh.
...
| `/journey` | **The book** — a real 3D book you can turn, one spread per chapter, with the same chapters written out below it. Plus the academic / work / volunteering timeline |
...
`chapters` adds a spread to the 3D book *and* a card to the written reader
...
- **Content-Security-Policy** — strict, with a fresh per-request nonce. No inline
  ...
  The bundle is tree-shaken to just the 22 exports the site uses — 119 KB gzipped
  instead of 168 KB.
...
- The chapters are written out in HTML below the 3D book and are always visible.
  Hidden text is discounted by search engines, and a two-page spread is
  unreadable at phone width — so the book is the presentation, not the content.
  It is also what renders when WebGL is unavailable.
- Scroll-reveal styles apply only under `html.js`, which `main.js` sets before
  paint. Content is visible by default, so a crawler or a failed script still
  sees a full page rather than an empty one.
...
public/
  ...
  vendor/              tree-shaken three.js bundle (committed)
scripts/
  three-entry.js       the three.js exports this site uses
  build-three.mjs      tree-shakes three into public/vendor (npm run build:three)
  check-three.mjs      guards that list against real usage (npm run check:three)
...
**→ [`DEPLOY.md`](DEPLOY.md) is the step-by-step walkthrough**, including pointing a
Hostinger domain at free hosting.

The short version: any Node host works. The repo ships a `Procfile`, an `.nvmrc` and
a [`render.yaml`](render.yaml) blueprint. Set `NODE_ENV=production`, `CSRF_SECRET`,
`SITE_ORIGIN` and the SMTP variables, then:
...
- Nothing is built on the host. `public/vendor/three.min.js` is a tree-shaken
  three.js bundle committed to the repo — rebuild it with `npm run build:three`
  after upgrading three or adding a `THREE.*` export.
- **Configure SMTP.** Most free hosts have an ephemeral disk, so the local-file
  fallback loses contact messages on every restart. The app warns loudly at boot in
  production if `SMTP_*` or `SITE_ORIGIN` are missing.
